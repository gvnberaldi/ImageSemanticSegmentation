...
from dlvc.utils import get_cnn_model, get_datasets, get_api_key, get_entity
...
def train(best_hyperparameters):
    ...
    logger = WandBLogger(api_key=get_api_key(), model=model, project_name="training", entity_name=get_entity(), run_name="CNN")
...
if __name__ == "__main__":
    ...
    logger = WandBHyperparameterTuning(api_key=get_api_key(), project_name="cnn_tuning", entity_name=get_entity())
    LOGGER = logger
    ...
    api = wandb.Api()
    runs = api.runs(f"{get_entity()}/cnn_tuning")
