...
from dlvc.utils import get_vit_model, get_datasets, get_api_key, get_entity
import wandb
...
def train(best_hyperparameter):
    ...
    logger = WandBLogger(api_key=get_api_key(), model=model, project_name="training", entity_name=get_entity(), run_name="ViT")
...
if __name__ == "__main__":
    logger = WandBHyperparameterTuning(api_key=get_api_key(), project_name="vit_tuning", entity_name=get_entity())
    LOGGER = logger
    ...
    api = wandb.Api()
    runs = api.runs(f"{get_entity()}/vit_tuning")
